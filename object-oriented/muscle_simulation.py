...
def objective(x,muscle_model,time_vector, active_force,reference_force):
    ...
    for i, (time,input,reference) in enumerate(zip(time_vector,active_force,reference_force)):
        ...
        if(i==1):
            ...
            error *= 1000
            error += np.sum((muscle_data[int(0.035/0.0001):int(0.3/0.0001)] - reference[int(0.035/0.0001):int(0.3/0.0001)])**2)
            error *= 1000
            error += np.sum((muscle_data[int(0.3/0.0001)::] - reference[int(0.3/0.0001)::])**2)
            error *= 1
        #error += 1/len(muscle_data) * np.sum((muscle_data-reference)**2)
        #error += np.sum((muscle_data-reference)**2)
        if(i==0):
...
def muscle_simulation():
    ...
    plt.plot(time_vector[0:0.3/0.0001],muscle_force[0:0.3/0.0001])
    plt.show()
