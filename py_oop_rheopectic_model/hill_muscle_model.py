...
class modified_hill_muscle_model():
    def __init__(self,km,kt,m,c,c1,cs,ks,ls0,delta,sim_dt) -> None:
        self.km = km
        self.kt = kt
        self.m = m
        self.c = c
    ...
    def _solve_muscle_dynamics(self,t,X,active_force):
        ...
        d2lm_dt = 1/self.m*(-self.c*dlm_dt-self.c1*dls_dt-self.km*lm+self.kt*(self.delta-lm)-active_force[int(t/self.sim_dt)])
    ...
    def get_parameters(self):
        ...
        x = [self.c,self.c1,self.cs,self.ks,self.ls0]
        return x
...
class rheopectic_modified_hill_muscle_model(modified_hill_muscle_model):
 # TODO: OVERWRITE METHODS
    def __init__(self, km,kt,m,cs,ks,ls0,c_rh,c_rh_min,c1,k1,k2,A,B,C,D,lambda0,F0,delta,sim_dt):
        super().__init__(km = km,kt = kt,m = m,c=0,c1 = c1,cs = cs, ks = ks,ls0 = ls0, delta = delta, sim_dt = sim_dt)
        self.k1 = k1
        self.k2 = k2
        self.A = A
        self.B = B
    ...
    def _solve_muscle_dynamics(self,t,X,active_force):
        ...
        d2lm_dt = 1/self.m*(-(self.c_rh * Lambda * dlm_dt + self.c_rh_min * dlm_dt + self.c1 * dls_dt) - self.km*(np.sign(lm) * (lm**2))+self.kt*(np.sign(self.delta-lm)* ((self.delta-lm)**2))-active_force[int(t/self.sim_dt)] - self.F0)
